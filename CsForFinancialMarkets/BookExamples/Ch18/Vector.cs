...
// 2012-6-4 DD using .NET 4.0 DLR
//
// (C) Datasim Component Technology 2005-2012
...
public class Vector<T> : Array<T>
{
  /*  private static BinaryOperatorT<T, T, T> addTT;
    ...
    /*    if( subTT == null )
        ...
        for( int i = v1.MinIndex; i <= v1.MaxIndex; i++ )
        {
            result[i] = NumericOperations.Subtract(v1[i], v2[(i - delta)]);
        }
        return result;
    }
    ...
    /*    if( subTT == null )
        ...
        for( int i = v1.MinIndex; i <= v1.MaxIndex; i++ )
        {
            result[i] = NumericOperations.Subtract(v1[i], t1);
        }
        return result;
    }
